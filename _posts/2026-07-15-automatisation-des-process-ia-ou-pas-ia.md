...
### Le piège de l'"IA partout" et le réflexe technologique

L'enthousiasme actuel pousse à vouloir "mettre de l'IA partout". On observe un réflexe systématique : les équipes pensent désormais à l'IA dès qu'on parle d'automatisation, simplement parce que c'est le sujet du moment. 

L'excitation autour de l'IA déclenche alors des véléités de traitement automatique, de tâche programmée, de classement de fichier... on imagine que l'IA est la solution à tout.
...
On pourra utiliser l'IA pour créer le script d'automatisation si on le souhaite, mais le process automatisé n'utilisera pas l'IA à chaque cycle. 
...
L'IA a une faiblesse structurelle : elle est **probabiliste**. Elle ne calcule pas, elle prédit le mot ou le token suivant le plus probable. Pour une rédaction d'email ou une synthèse de document, c'est un atout majeur. Pour le calcul d'une remise commerciale ou la vérification d'une conformité réglementaire, c'est un risque critique.
...
C'est une tâche où il y a de l'interprétation, du langage naturel, ou où le résultat peut varier légèrement tout en restant correct.
* **L'indice :** Vous ne pouvez pas écrire de règle exhaustive, vous devez "juger" ou "comprendre".
* **L'outil :** Un LLM c'est-à-dire un modèle d'IA.
* **La valeur :** Capacité de synthèse, d'analyse et d'adaptation.
* **L'erreur :** Vouloir que l'IA soit déterministe. L'IA ne "sait" pas, elle "estime", elle "prédit l'output le plus plausible".
